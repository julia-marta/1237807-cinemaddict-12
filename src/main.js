...
render(header, createProfileMarkup(), `beforeend`);
render(main, createNavigationMarkup(), `afterbegin`);
render(main, createSortingMarkup(), `beforeend`);
render(main, createFilmsMarkup(), `beforeend`);
...
Array.from(filmsContainers).forEach((item) => {
  for (let i = 0; i < (item.parentElement.classList.contains(`films-list--extra`) ? FILM_EXTRA_COUNT : FILM_CARDS_COUNT); i++) {
    render(item, createFilmCardMarkup(), `beforeend`);
  }
});

render(filmsList, createShowButtonMarkup(), `beforeend`);

const statistics = footer.querySelector(`.footer__statistics`);

render (statistics, createStatisticsMarkup(), `beforeend`);
